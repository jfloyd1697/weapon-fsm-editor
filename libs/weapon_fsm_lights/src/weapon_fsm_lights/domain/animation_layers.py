...
class LightLayerDef(DataClassDictMixin):
    ...
    start_ms: int = field(
        default=0,
        metadata={"editor": "int", "min": 0, "max": 10_000_000},
    )
    duration_ms: int = field(
        default=1000,
        metadata={"editor": "int", "min": 1, "max": 10_000_000},
    )
    color: str = field(
        default="#ffffff",
        metadata={"editor": "color"},
    )
    intensity: float = field(
        default=1.0,
        metadata={"editor": "float", "min": 0.0, "max": 1.0, "step": 0.05},
    )

    reverse: bool = field(
        default=False,
        metadata={"editor": "bool", "label": "Reverse layer"},
    )
    invert: bool = field(
        default=False,
        metadata={"editor": "bool", "label": "Invert light / dark"},
    )

    mask_enabled: bool = field(
        default=False,
        metadata={"editor": "bool", "label": "Use radial mask"},
    )
    mask_center: list[float] = field(
        default_factory=lambda: [0.5, 0.5],
        metadata={"editor": "point2", "min": 0.0, "max": 1.0, "step": 0.01},
    )
    mask_radius: float = field(
        default=1.0,
        metadata={"editor": "float", "min": 0.0, "max": 10.0, "step": 0.01},
    )
    mask_softness: float = field(
        default=0.0,
        metadata={"editor": "float", "min": 0.0, "max": 10.0, "step": 0.01},
    )
    mask_invert: bool = field(
        default=False,
        metadata={"editor": "bool", "label": "Invert radial mask"},
    )
...
class SolidLayerDef(LightLayerDef):
    type: LightLayerType = field(
        default=LightLayerType.SOLID,
        metadata={"editor": "layer_type"},
    )
...
class RadialPulseLayerDef(LightLayerDef):
    type: LightLayerType = field(
        default=LightLayerType.RADIAL_PULSE,
        metadata={"editor": "layer_type"},
    )
...
class WipeLayerDef(LightLayerDef):
    ...
    speed: float = field(
        default=1.0,
        metadata={"editor": "float", "min": 0.0, "max": 100.0, "step": 0.05},
    )
...
class ChaseLayerDef(LightLayerDef):
    ...
    led_count: int = field(
        default=1,
        metadata={"editor": "int", "min": 1, "max": 9999},
    )
...
class SparkleLayerDef(LightLayerDef):
    type: LightLayerType = field(
        default=LightLayerType.SPARKLE,
        metadata={"editor": "layer_type"},
    )

    seed: int = field(
        default=1,
        metadata={"editor": "int", "min": 0, "max": 2_147_483_647},
    )
    density: float = field(
        default=0.2,
        metadata={"editor": "float", "min": 0.0, "max": 1.0, "step": 0.05},
...
def normalize_layer_dict(data: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized.setdefault("reverse", False)
    normalized.setdefault("invert", False)

    normalized.setdefault("mask_enabled", False)
    normalized.setdefault("mask_center", [0.5, 0.5])
    normalized.setdefault("mask_radius", 1.0)
    normalized.setdefault("mask_softness", 0.0)
    normalized.setdefault("mask_invert", False)
