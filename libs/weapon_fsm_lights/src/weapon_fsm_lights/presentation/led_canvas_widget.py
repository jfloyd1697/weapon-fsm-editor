...
class LedCanvasWidget(QWidget):
    ...
    def set_asset(self, asset: LightSequenceAsset, *, sequence_name: str = "layout", mode: str = "once") -> None:
        self.show_sequence(asset, sequence_name=sequence_name, mode=mode)

    def show_sequence(self, asset: LightSequenceAsset, *, sequence_name: str, mode: str) -> None:
        ...
        if asset.frames:
            self.set_frame(0, asset.frames[0])
        self.update()
    ...
    def set_frame(self, frame_index: int, frame: LightFrame) -> None:
    ...
    def _move_led(self, led_id: str, x: float, y: float) -> None:
        ...
        updated = [replace(led, x=x, y=y) if led.id == led_id else led for led in self._asset.leds]
...
class LedPreviewPanel(QWidget):
    ...
    def set_example_layout_path(self, path: str | Path | None) -> None:
        ...
        self.example_button.setEnabled(
            self._example_layout_path is not None and self._example_layout_path.exists()
        )
    ...
    def play_sequence(
        ...
        *,
        ...
        self.canvas.show_sequence(asset, sequence_name=sequence_name, mode=mode)
        self.player.set_asset(asset, sequence_name=sequence_name, mode=mode, autoplay=True)
    ...
    def load_layout_path(self, path: str | Path) -> None:
        ...
        sequence_name = self._current_asset_path.stem
        self.canvas.set_asset(asset, sequence_name=sequence_name, mode="once")
        self.player.set_asset(asset, sequence_name=sequence_name, mode="once", autoplay=False)
