...
class ProfileValidator:
    ...
    def _validate_triggers(self, gun: GunConfig, weapon: WeaponConfig) -> list[ValidationIssue]:
        ...
        for transition in weapon.transitions:
            if transition.trigger not in available_events:
                issues.append(ValidationIssue(f"weapon.transitions.{transition.id}.trigger", f"Unknown trigger '{transition.trigger}'"))
        return issues
    ...
    def _validate_assets(self, weapon: WeaponConfig) -> list[ValidationIssue]:
        ...
        for name, clip_set in weapon.clip_sets.items():
            ...
            if clip_set.mode not in {"random", "random_no_repeat", "sequence"}:
                issues.append(ValidationIssue(f"clip_sets.{name}.mode", f"Clip set '{name}' has invalid mode '{clip_set.mode}'"))
    ...
    def _validate_guards(self, weapon: WeaponConfig, context: ValidationContext) -> list[ValidationIssue]:
        issues: list[ValidationIssue] = []
        for transition in weapon.transitions:
            issues.extend(self._validate_guard(transition.guard, f"weapon.transitions.{transition.id}.guard", context))
        return issues

    def _validate_guard(self, guard: GuardDef | None, path: str, context: ValidationContext) -> list[ValidationIssue]:
        issues: list[ValidationIssue] = []
        if guard is None:
            return issues
        for name, spec in (("var_eq", guard.var_eq), ("var_gt", guard.var_gt), ("var_gte", guard.var_gte), ("var_lt", guard.var_lt), ("var_lte", guard.var_lte)):
            if spec is not None:
                issues.extend(self._validate_guard_compare(spec, f"{path}.{name}", context))
    ...
    def _validate_guard_compare(self, spec: dict[str, object], path: str, context: ValidationContext) -> list[ValidationIssue]:
        issues: list[ValidationIssue] = []
        name = spec.get("name")
        if not name:
    ...
    def _validate_actions(self, weapon: WeaponConfig, context: ValidationContext) -> list[ValidationIssue]:
        issues: list[ValidationIssue] = []
        for state in weapon.states.values():
            for index, action in enumerate(state.on_entry):
                issues.extend(self._validate_action(action, f"weapon.states.{state.id}.on_entry.{index}", context))
            for index, action in enumerate(state.on_exit):
                issues.extend(self._validate_action(action, f"weapon.states.{state.id}.on_exit.{index}", context))
        for transition in weapon.transitions:
            for index, action in enumerate(transition.actions):
                issues.extend(self._validate_action(action, f"weapon.transitions.{transition.id}.actions.{index}", context))
        return issues

    def _validate_action(self, action: ActionDef, path: str, context: ValidationContext) -> list[ValidationIssue]:
        return [ValidationIssue(path, message) for message in RuntimeCommand.validate_action(action, context)]
